...
class TrainFunction:
    ...
    @property
    def is_toggle(self):
        return self.default_duration is None

    def __repr__(self):
        return self.name + ":" + str(self.id)


LIGHT = TrainFunction('Licht', 0, False, (TAG_DEFAULT_LIGHT,), cooldown=1., default_duration=None)
SLOW_MODE = TrainFunction("Langsam-Modus", 3, False, (), cooldown=0, default_duration=None)
INSTANT_ACCELERATION = TrainFunction("Instantane Beschleunigung", 4, True, (), cooldown=0, default_duration=None)
SOUND = TrainFunction("Motor", 2, False, (TAG_DEFAULT_SOUND,), cooldown=3., default_duration=None)
...
class TrainInfo:
    ...
    max_speed_by_track: Dict[str, float] = field(default_factory=lambda: {'regional': 160})
    ...
    @cached_property
    def speed_limit_unknown_track(self) -> Optional[float]:
        return None if not self.max_speed_by_track else min(self.max_speed_by_track.values())
...
class Train:
    ...
    def image(self) -> Image:
        return Image.open(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'assets', 'images', 'train_thumbnails', self.img_path)) if self.img_path else None
...
SHUTTLE_ =             TrainInfo("Shuttle", "🚉", .6, max_delay=40, delay_rate=.4, max_speed_in_station=(65, 65, 70))
...
E_BW = Train(INTERREGIO_BLAU, "BR 101", "Märklin 37394", 1, (0, 13, 25, 46, 67, 86, 108, 125, 140, 156, 173, 191, 201, 215, 226), 30., img_path="E-Lok BW.png", stop_by_mm1_reverse=False,
             functions=(LIGHT, TrainFunction("Nebelscheinwerfer", 2, False, (TAG_SPECIAL_LIGHT,), 1., None), TrainFunction("Fahrtlicht hinten", 3, False, (TAG_SPECIAL_LIGHT,), 1., None), INSTANT_ACCELERATION))
...
SHUTTLE = Train(SHUTTLE_, "Shuttle", "Märklin 36967", 65, speeds(140, 1.5), 35., DrivingModel(reverse_time_with_sound=4.), img_path="Shuttle.png",
                functions=(LIGHT, TrainFunction("Innenbeleuchtung", 1, False, (TAG_SPECIAL_LIGHT,), 1., None), SOUND, TrainFunction("Horn", 3, False, (TAG_SPECIAL_SOUND,)), INSTANT_ACCELERATION))
...
BUS = Train(BUS_, "VT95", "Märklin H0 3016", 62, speeds(190), 40., img_path="Schienenbus.png", stop_by_mm1_reverse=False, functions=(LIGHT,))
...
DIESEL = Train(GUETER, "V 60 (BR 360)", "Märklin 29155", 72, speeds(180), 25., img_path="Diesel.png",
               functions=(LIGHT, SLOW_MODE, INSTANT_ACCELERATION))
E40 = Train(GUETER, "BR E40", "Märklin 39140", 23, speeds(280, 1.0), 30., img_path="E40.png", stop_by_mm1_reverse=False,
...
TRAINS = [ICE, S, SHUTTLE, BUS, E_RB, E_BW, ROT, BEIGE, E40, DIESEL, DAMPF]
